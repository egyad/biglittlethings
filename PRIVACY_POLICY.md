# Privacy Policy for Big Little Things

**Last Updated: February 2, 2026**

## Introduction

Welcome to Big Little Things ("we," "our," or "us"). We are committed to protecting your privacy and the privacy of your family. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application.

**By using Little Things, you agree to the collection and use of information in accordance with this policy.**

---

## Information We Collect

### Personal Information You Provide

**Account Information:**
- Email address
- Display name
- Profile photo (optional)
- Child's name and birth date
- Village member information (names, emails, roles)

**Content You Create:**
- Photos of your child
- Voice recordings
- Written captions and journal entries
- Comments ("Echoes") on memories
- Vault items and time-lock settings

**Payment Information:**
- Subscription status and type (processed by Apple/Google)
- We do NOT store credit card information (handled by App Store/Play Store)

### Automatically Collected Information

**Device Information:**
- Device type and model
- Operating system version
- Unique device identifiers
- Mobile network information

**Usage Data:**
- App features used
- Time spent in app
- Crash reports and error logs
- Performance metrics

**Authentication Data:**
- Login timestamps
- Authentication tokens
- Session information

---

## How We Use Your Information

We use your information to:

### Provide Core Services
- Create and manage your account
- Store and display your family memories
- Enable Village sharing with invited family members
- Process voice recordings and photos
- Manage Vault time-locked content
- Sync data across your devices

### Improve Our Service
- Analyze app usage to improve features
- Fix bugs and technical issues
- Develop new features
- Optimize app performance

### Communicate With You
- Send important service updates
- Respond to support requests
- Send subscription renewal reminders
- Notify about new features (if opted in)

### Process Payments
- Manage subscription status
- Process upgrades and downgrades
- Handle refunds (through Apple/Google)

### Legal Compliance
- Comply with legal obligations
- Enforce our Terms of Service
- Protect against fraud and abuse

---

## How We Share Your Information

### We Do NOT Sell Your Data
We will never sell, rent, or trade your personal information or family photos to third parties.

### Limited Sharing

**With Your Village Members:**
- Photos, voice notes, and captions you create are shared with Village members you invite
- Village members can see content based on permissions you set
- Vault items remain hidden until unlock date

**With Service Providers:**
- **Firebase (Google):** Cloud storage, authentication, database
- **RevenueCat:** Subscription management
- **Apple/Google:** Payment processing
- These providers are contractually obligated to protect your data

**For Legal Reasons:**
- If required by law or legal process
- To protect our rights or property
- To prevent fraud or abuse
- In connection with a business transfer (merger, acquisition)

**With Your Consent:**
- Any other sharing will only occur with your explicit permission

---

## Data Storage and Security

### Where We Store Your Data

**Cloud Storage:**
- Data stored on Firebase (Google Cloud Platform)
- Servers located in: us-central1
- Compliant with GDPR, CCPA, and other privacy regulations

**Local Storage:**
- Temporary cache on your device
- Automatically cleared when you log out

### Security Measures

**Encryption:**
- All data encrypted in transit (TLS/SSL)
- All data encrypted at rest (AES-256)
- End-to-end encryption for sensitive content

**Access Controls:**
- Multi-factor authentication available
- Role-based access for Village members
- Regular security audits
- Automatic session timeout

**Monitoring:**
- 24/7 security monitoring
- Intrusion detection systems
- Regular vulnerability assessments

### Data Retention

**Active Accounts:**
- Data retained as long as your account is active
- Memories stored indefinitely (that's the point!)

**Inactive Accounts:**
- Accounts inactive for 2 years may be archived
- You'll receive email notification before archival
- Data can be restored upon request

**Deleted Accounts:**
- Data permanently deleted within 30 days of account deletion
- Backups purged within 90 days
- Some data may be retained for legal compliance

---

## Your Privacy Rights

### Access and Control

**You Have the Right To:**
- Access all your personal data
- Download your data (Export feature)
- Correct inaccurate information
- Delete your account and data
- Opt out of marketing communications
- Manage Village member access

### How to Exercise Your Rights

**In-App:**
- Settings → Privacy → Manage Data
- Settings → Account → Delete Account
- Settings → Village → Manage Members

**By Email:**
- Contact: egyadhitama@gmail.com
- We'll respond within 30 days

### Regional Rights

**For EU Users (GDPR):**
- Right to data portability
- Right to be forgotten
- Right to restrict processing
- Right to object to processing
- Right to lodge a complaint with supervisory authority

**For California Users (CCPA):**
- Right to know what data is collected
- Right to delete personal information
- Right to opt-out of data sales (we don't sell data)
- Right to non-discrimination

**For Other Regions:**
- We extend similar rights to all users globally

---

## Children's Privacy

### Age Requirements
- Big Little Things is designed for parents to document their children
- Users must be 18+ to create an account
- We do not knowingly collect data from children under 13

### Children's Data
- Photos and information about children are provided by parents
- Parents control all data about their children
- Children cannot create accounts or access the app independently

### COPPA Compliance
- We comply with the Children's Online Privacy Protection Act
- Parents have full control over their children's information
- No advertising targeted at children

---

## Cookies and Tracking

### What We Use

**Essential Cookies:**
- Authentication tokens
- Session management
- Security features

**Analytics (Optional):**
- Firebase Analytics (can be disabled in settings)
- Crash reporting (Crashlytics)
- Performance monitoring

**We Do NOT Use:**
- Advertising cookies
- Third-party tracking pixels
- Cross-site tracking
- Behavioral advertising

### Your Choices
- Disable analytics in Settings → Privacy
- Use "Do Not Track" browser settings (web version)
- Clear app cache to remove local data

---

## Third-Party Services

### Services We Use

**Firebase (Google):**
- Purpose: Authentication, database, storage
- Privacy Policy: https://firebase.google.com/support/privacy

**RevenueCat:**
- Purpose: Subscription management
- Privacy Policy: https://www.revenuecat.com/privacy

**Apple Sign-In:**
- Purpose: Authentication
- Privacy Policy: https://www.apple.com/legal/privacy/

**Google Sign-In:**
- Purpose: Authentication
- Privacy Policy: https://policies.google.com/privacy

### Third-Party Links
- Our app may contain links to external websites
- We are not responsible for their privacy practices
- Review their privacy policies before providing information

---

## International Data Transfers

**Cross-Border Transfers:**
- Your data may be transferred to and stored in countries outside your residence
- We use standard contractual clauses approved by the EU Commission
- We ensure adequate protection regardless of location

**EU-US Data Transfers:**
- We comply with GDPR requirements
- Standard contractual clauses in place
- Adequate safeguards for data protection

---

## Changes to This Privacy Policy

**Updates:**
- We may update this policy periodically
- Material changes will be notified via email and in-app notification
- Continued use after changes constitutes acceptance
- Previous versions available upon request

**Notification:**
- 30 days notice for material changes
- Immediate notification for changes affecting your rights
- Option to delete account if you disagree with changes

---

## Data Breach Notification

**In Case of Breach:**
- We'll notify affected users within 72 hours
- Notification via email and in-app alert
- Details of what data was affected
- Steps we're taking to address the breach
- Recommendations for protecting your account

---

## Contact Us

### Privacy Questions

**Email:** egyadhitama@gmail.com


**Response Time:** Within 30 days



### Support

**Email:** egyadhitama@gmail.com

---

## Your Consent

By using Little Things, you consent to:
- Collection and use of information as described
- Transfer of data to service providers
- Storage of data in the cloud
- Processing of data for app functionality

You may withdraw consent by deleting your account.

---

## Summary (TL;DR)

✅ **We collect:** Photos, voice notes, names, emails
✅ **We use it for:** Storing your memories, enabling Village sharing
✅ **We share with:** Only your invited Village members and essential service providers
✅ **We protect it with:** Encryption, secure servers, access controls
✅ **You control:** Who sees what, data export, account deletion
❌ **We DON'T:** Sell your data, show ads, track you across apps

---


*This privacy policy is effective as of February 2, 2026.*
